...
async function getUserFromToken(authHeader: string | undefined) {
  ...
  // التحقق من أن authHeader هو string
  if (typeof authHeader !== 'string') {
    console.error('❌ [AI Assistant] authHeader is not a string:', typeof authHeader);
    return null;
  }
  ...
  try {
    ...
    const stats = {
      ...
      courses: {
        ...
        active: allCourses?.filter(c => c.is_active).length || 0,
        inactive: allCourses?.filter(c => !c.is_active).length || 0,
        byLevel: {} as Record<string, number>
      },
      registrations: {
...
• وزيع حسب التخصص: ${Object.entries(stats.students?.byMajor || {}).map(([k, v]) => `${k}: ${v}`).join(', ')}
...
⚠️ ملاحظات مهمة:
• استخدم فقط الأقام الحقيقية من البيانات أعلاه
