    ...
    try {
      ...
      try {
        const deleteResult = await fetchJSON(
          `https://${projectId}.supabase.co/functions/v1/make-server-1573e40a/admin/users/${selectedStudent.user_id}`,
          {
            method: 'DELETE',
            headers: {
              Authorization: `Bearer ${publicAnonKey}`,
        ...
        if (deleteResult.success) {
          console.log('✅ [ManageStudents] Student deleted from backend');
          
          toast.success(
            language === 'ar'
              ? '✅ تم حذف الطالب بنجاح من قاعدة البيانات'
              : '✅ Student deleted successfully from database',
            {
              description: language === 'ar'
                ? '🗑️ تم الحذف النهائي من Supabase'
                : '🗑️ Permanently deleted from Supabase'
            }
          );
        ...
        console.log('⚠️ [ManageStudents] Backend delete failed, trying localStorage...');
      }
