...
export const StudentDashboard: React.FC = () => {
  ...
  const refreshUserData = async () => {
    try {
      ...
      if (!userInfo || !userInfo.id) {
        console.error('❌ [Dashboard] userInfo is missing or invalid:', userInfo);
        toast.error(
          language === 'ar'
            ? 'خطأ: بيانات المستخدم غير متوفرة'
      ...
      console.log('📊 [Dashboard] Fetching fresh user data from database...');
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.user) {
            console.log('✅ [Dashboard] Got fresh user data from SQL:', data.user);
            console.log('📊 [Dashboard] Major:', data.user.major);
            console.log('📊 [Dashboard] Level:', data.user.level);
            console.log('📊 [Dashboard] GPA:', data.user.gpa);
            console.log('📊 [Dashboard] Student ID:', data.user.student_id);
            
            // تحديث userInfo في الـ Context
            const updatedUserInfo = {
              ...userInfo,
              ...data.user,
              major: data.user.major || data.user.students?.[0]?.major,
              level: data.user.level || data.user.students?.[0]?.level,
              gpa: data.user.gpa || data.user.students?.[0]?.gpa,
              total_credits: data.user.total_credits || data.user.students?.[0]?.total_credits || 0,
              completed_credits: data.user.completed_credits || data.user.students?.[0]?.completed_credits || 0,
            };
            ...
            setRefreshedUserData(data.user);
            
            toast.success(
              language === 'ar'
                ? '✅ تم تحميل البيانات من قاعدة البيانات'
        ...
        console.warn('⚠️ [Dashboard] Backend fetch failed, using localStorage:', backendError);
      }

      // 🔄 FALLBACK: استخدام localStorage إذا فشل Backend
      console.log('💾 [Dashboard] Using localStorage as fallback...');
  ...
  const fetchStatistics = async () => {
    try {
      ...
      // ✅ التحقق من وجود access token و userInfo
      if (!accessToken) {
      ...
      if (!userInfo || !userInfo.id) {
        console.warn('⚠️ [Dashboard] No userInfo or userInfo.id for statistics');
        return;
      }
      ...
      if (!uuidRegex.test(userId)) {
        console.error('❌ [Dashboard] Invalid user ID format for statistics:', userId);
        return;
      }
      ...
      try {
        const response = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-1573e40a/dashboard/student/${userId}`,
          {
            headers: {
              Authorization: `Bearer ${publicAnonKey}`,
            },
        ...
        // ✅ صامت - لا نعرض أي شيء
        console.log('🔄 [Dashboard] Backend offline, calculating locally');
      }
